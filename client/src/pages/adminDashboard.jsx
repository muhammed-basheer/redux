    ...
    return (
        ...
        <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="overflow-x-auto sm:overflow-x-hidden">
            <table className="w-full table-auto sm:table-fixed border-collapse bg-white shadow-md rounded-lg overflow-hidden">
              <thead className="bg-blue-500 text-white">
                <tr>
                  <th className="p-3 text-left text-xs sm:text-base">Username</th>
                  <th className="p-3 text-left text-xs sm:text-base">Email</th>
                  <th className="p-3 text-left text-xs sm:text-base">Actions</th>
                </tr>
              </thead>
              <tbody>
                {!users ? (
                  <tr>
                    <td colSpan="3" className="p-3 text-center">No users found.</td> // Show this if the users array is empty
                  </tr>
                ) : (
                  users.map((user) => (
                    <tr key={user._id} className="border-b hover:bg-gray-100">
                      <td className="p-3 text-xs sm:text-base">
                        <input onChange={(e) => setUpdateUser({ ...updateUser, username: e.target.value })} type="text" defaultValue={user.username} />
                      </td>
                      <td className="p-3 text-xs sm:text-base">
                        <input onChange={(e) => setUpdateUser({ ...updateUser, email: e.target.value })} type="email" defaultValue={user.email} />
                      </td>
